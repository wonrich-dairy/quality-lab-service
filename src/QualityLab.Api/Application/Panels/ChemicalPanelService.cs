...
public sealed class ChemicalPanelService : IChemicalPanelService
{
    ...
    public async Task<ChemicalPanel> RecordPanelAsync(string batchCode, RecordPanelRequest request, string userId, CancellationToken ct)
    {
        ...
        // QA-20-02: Reject panels before run completion
        if (batch.Status == BatchStatus.AwaitingPanel && batch.CompletionTimeUtc == null)
            throw new InvalidOperationException($"Batch '{batchCode}' has a processing run that has not completed. Panels cannot be recorded until processing is finished.");
        ...
        var panel = new ChemicalPanel
        {
            ...
            FatPercent = Math.Round(request.FatPercent, 2, MidpointRounding.AwayFromZero),
            LactometerReading = request.LactometerReading.HasValue ? Math.Round(request.LactometerReading.Value, 2, MidpointRounding.AwayFromZero) : null,
            TemperatureCelsius = request.TemperatureCelsius.HasValue ? Math.Round(request.TemperatureCelsius.Value, 2, MidpointRounding.AwayFromZero) : null,
            Ph = Math.Round(request.Ph, 2, MidpointRounding.AwayFromZero),
            CorrectedClr = correctedClr.HasValue ? Math.Round(correctedClr.Value, 2, MidpointRounding.AwayFromZero) : null,
            Snf = snf.HasValue ? Math.Round(snf.Value, 2, MidpointRounding.AwayFromZero) : null,
            Ts = ts.HasValue ? Math.Round(ts.Value, 2, MidpointRounding.AwayFromZero) : null,
    ...
    private static void ValidateForProductLine(ProductLine productLine, RecordPanelRequest request)
    {
        ...
        // QA-20-03: Fat% is required (0 or missing is invalid on all lines)
        if (request.FatPercent <= 0)
            throw new InvalidOperationException("Fat percent is required and must be greater than 0.");

        // Validate physical ranges
        if (request.FatPercent > 15)
            throw new InvalidOperationException("Fat percent must be between 0 and 15. Permitted range: 0.01–15.00.");
        ...
        if (productLine.IsLiquid())
        {
            ...
            // QA-20-06: -3.0 must be rejected (boundary is exclusive)
            if (request.TemperatureCelsius <= -3m || request.TemperatureCelsius > 50)
                throw new InvalidOperationException("Temperature must be above -3.0 and at most 50 °C. Permitted range: (-3.0, 50.0].");
        }
        else
        {
